...
def draw_text(screen,text:str,center:tuple[int,int],max_diameter:int,text_color:str) -> None:
    if max_diameter <= 5:
        return
    ...
    while font_size > 6:
        ...
        if text_surf.get_width() <= max_diameter - 4 and text_surf.get_height() <= max_diameter - 4:
            break
        font_size -= 1
    font = pygame.font.SysFont("Arial", font_size, bold=True)
    text_surf = font.render(text, True, text_color)
...
def to_screen_coords(x_graph: float, y_graph: float, window_height: int, scale: float = 20, offset_x: int = 50, offset_y: int = 50) -> tuple[int, int]:
...
def hub_coords_on_screen(graph: Graph, zoom_modifier: float, camera_x: float, camera_y: float) -> dict:
    hubs = graph.zones
    ...
    for hub_name, (hx, hy) in scaled_coords.items():
        x, y = to_screen_coords(hx, hy, window_height=720, scale=scale, offset_x=offset_x, offset_y=offset_y)
    ...
    for cord,set_id in draw_in_hub.items():
        if len(set_id) == 1:
            ...
            for i,drone_id in enumerate(set_id):
                ...
                render_drones(screen, int(12 * zoom_modifier),(drone_x,drone_y),drone_id)


def draw_sim(screen,step:list[str],hub_coords_on_screen:dict,drones_pos:dict,map_data:MapData,zoom_modifier:float,graph:Graph) -> None:
...
def update_drones_cords(step:list[str],hub_coords_on_screen:dict,drones_pos:dict,map_data:MapData,zoom_modifier:float,graph:Graph) -> None:
    doing = {}
    for log in step:
...
def vizualizer(graph: Graph,solution_logs:list[str],nb_drones:int,map_data:MapData) -> None:
    pygame.init()
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                update_drones_cords(current_step,coords_on_screen,drones_pos,map_data,zoom_modifier,graph)
            elif event.type == pygame.KEYDOWN:
                if event.key == pygame.K_s:
                    sim_activated = not sim_activated
                    if sim_activated:
                        logs = (a for a in solution_logs)
                        coords = hub_coords_on_screen(graph, zoom_modifier, camera_x, camera_y)
                        drones_pos = init_drones(nb_drones, coords[map_data.start_hub_name], zoom_modifier)
        ...
        if sim_activated and current_step is not None:
            draw_sim(screen,current_step,coords_on_screen,drones_pos,map_data,zoom_modifier,graph)
        else:
