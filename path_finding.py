...
class Drone(BaseModel):
    ...
    def decide_next_step(self,graph:Graph,map_data:MapData,world_state:"WorldState") -> str:
        g = {}
        heap = []
        ...
        while len(heap) != 0:
            ...
            for neighbor in graph.get_neighbors(active_zone):
                if world_state.is_available(neighbor) != True:
                    continue
...
class WorldState(BaseModel):
    ...
    def build(cls,graph:Graph,map_data:MapData) -> "WorldState":
        ...
        for connection in map_data.connections:
            connection_info[f"{connection.connection_from}-{connection.connection_to}"] = 0
        connection_reservations = {}
        for connection in map_data.connections:
            connection_reservations[f"{connection.connection_from}-{connection.connection_to}"] = 0
        zone_reservations = {}
        for zone in map_data.hubs:
            zone_reservations[f"{zone.hub_name}"] = 0
        main_data = {"zone_info":zone_info,
    ...
    def is_available(self,zone:str,graph:Graph) -> bool:
        return graph.is_zone_available(zone,self.zone_info[zone])
...
class Simulation(BaseModel):
    ...
    move_counter:int = Field(default=0)
    output_logs:list = Field(default=[])
